...
def absdiff(vecs, normalize=True):
    x, y = vecs
    if normalize:
...
def get_siamese_model():
    ...
    distance = Lambda(absdiff,
                      output_shape=absdiff_output_shape)([vector_1, vector_2])
    print('distance.shape: ', distance)

    outputs = Dense(1)(distance)
    outputs = Activation("sigmoid")(outputs)
    model = Model(inputs=[input_tensor_1, input_tensor_2], output=outputs)
    return model
...
csv_logger = CSVLogger("model_history_log_2.csv", append=True)
